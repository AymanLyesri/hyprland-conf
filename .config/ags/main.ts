    ...
    ws.map(({ id }) =>
    {
      if (id == -99) {
        return Widget.Button({
          on_clicked: () => hyprland.messageAsync("dispatch workspace special"),
          child: Widget.Icon({ icon: "view-grid-symbolic" }),
...
function Media()
{
  const label = Widget.Label({
    label: Utils.watch("", mpris, "player-changed", () =>
    {
      if (mpris.players[0]) {
  ...
  return Widget.Box({ children: [trigger, revealer] })
}
...
function Bandwidth()
{
  const bandwidth = Variable("", {
    ...
    listen: [App.configDir + '/bandwidth.sh', out =>
    {
...
function Right()
{
  return Widget.Box({
    hpack: "end",
    spacing: 8,
    children: [Bandwidth(), Volume(), BatteryLabel(), Clock(), SysTray()],
  });
}
