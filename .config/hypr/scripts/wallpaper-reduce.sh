...
input_directory="$HOME/wallpapers/normal"  # Change to your directory path
output_directory="$HOME/wallpapers/normal" # Change to your desired output path
...
for input_image in "$input_directory"/*; do
    ...
    if [[ $input_image =~ \.(jpg|jpeg|png)$ ]]; then
        ...
        if [[ $input_image =~ \.png$ ]]; then
            ...
            magick "$input_image" -strip -quality 100 -define png:compression-level=9 "$output_image"
            current_size=$(stat -c%s "$output_image")
        else
            ...
            while true; do
                ...
                if [ $current_size -le $target_size ]; then
                    echo "Processed: $filename - Size: $current_size bytes (target: $target_size bytes)"
                    break
                fi
